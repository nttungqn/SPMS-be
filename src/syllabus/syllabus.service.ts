...
export class SyllabusService {
  ...
  async findAll(filter:GetSyllabusFilterDto):Promise<Syllabus[]|any> {
    ...
    const query =this.syllabusRepository.createQueryBuilder('sy')
    .leftJoinAndSelect('sy.subject','subject').where(qb=>{search? qb.where('(subject.TenTiengViet LIKE :search OR subject.TenTiengAnh LIKE :search)',{search: `%${search}%`}):{}})
  ...
  async findOne(id: number):Promise<Syllabus> {
    const found=await this.syllabusRepository.findOne(id,{relations:["author","schoolYear","typeOfEdu","updateBy","subject"],where:{isDeleted:false}});
    if(!found){
  ...
  async isExist(createSyllabusDto:CreateSyllabusDto):Promise<boolean>{
    ...
    const found=await this.syllabusRepository.findOne({where:{schoolYear,subject,typeOfEdu,isDeleted:false}})
